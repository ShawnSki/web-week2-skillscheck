...
myNumbers.forEach(function(item, ind) {
...
let enemies = people.filter(function(val, indx, arr) {
	if (people[indx].friend === false){
	  return people[indx];
	}
  })
...
let totallyAwesome = people.reduce(function(acc, cur, curInx) {
	// return cur + people[curInx].awesomeLevel;
	return acc + people[curInx].awesomeLevel;
	}, 0);
